"""
Scraper for Moltbook - indexes agents from high-signal submolts.
Priority submolts: builds, agents, tooling, introductions, openclaw-explorers
"""
import httpx
import asyncio
...
# High-signal submolts for finding agents with real work
PRIORITY_SUBMOLTS = ["builds", "agents", "tooling", "openclaw-explorers", "introductions", "general"]
SUBMOLT_POST_LIMITS = {
    "builds": 500,
    "agents": 500,
    "tooling": 300,
    "openclaw-explorers": 300,
    "introductions": 300,
    "general": 100,
}

DOMAIN_TAGS = {
    "coding": ["code", "python", "javascript", "typescript", "rust", "golang", "api", "github", "deploy", "docker", "backend", "frontend", "database", "sql", "programming", "dev", "software", "build"],
    "research": ["research", "paper", "study", "analysis", "data", "science", "experiment", "hypothesis", "findings", "arxiv", "academic", "report"],
    "writing": ["writing", "blog", "article", "post", "essay", "draft", "edit", "content", "story", "narrative", "docs"],
    "automation": ["automation", "workflow", "cron", "schedule", "pipeline", "script", "task", "job", "trigger", "orchestrat", "agentic"],
    ...
    "finance": ["finance", "trading", "market", "stock", "crypto", "investment", "portfolio", "price", "economic", "defi"],
    "productivity": ["productivity", "calendar", "email", "meeting", "schedule", "organiz", "plan", "assistant"],
    ...
    "security": ["security", "pentest", "ctf", "exploit", "vulnerability", "bug bounty", "hacking", "cipher"],
    "infrastructure": ["infrastructure", "devops", "kubernetes", "terraform", "cloud", "deployment", "server", "container"],
}
...
async def fetch_posts_from_submolt(client: httpx.AsyncClient, submolt: str, max_posts: int) -> list[dict]:
    """Fetch posts from a specific submolt."""
    posts = []
    cursor = None
    while len(posts) < max_posts:
        params = {"limit": min(100, max_posts - len(posts)), "submolt": submolt, "sort": "top"}
        if cursor:
        ...
        try:
            ...
            logger.warning(f"Error fetching {submolt}: {e}")
            break
...
async def fetch_agent_profile(client: httpx.AsyncClient, name: str) -> Optional[dict]:
    """Fetch full agent profile from Moltbook."""
    try:
...
async def scrape_all_agents(max_posts_total: int = 3000, enrich_github: bool = True) -> dict:
    """Scrape agents from priority submolts, enrich with profiles and GitHub."""
    ...
    async with httpx.AsyncClient() as client:
        for submolt in PRIORITY_SUBMOLTS:
            limit = min(SUBMOLT_POST_LIMITS.get(submolt, 200), max_posts_total // len(PRIORITY_SUBMOLTS))
            logger.info(f"Scraping submolt: {submolt} (limit={limit})")
            posts = await fetch_posts_from_submolt(client, submolt, limit)
            ...
            logger.info(f"  → {len(posts)} posts, {len(agents)} unique agents total")
            await asyncio.sleep(0.5)

    # Fetch full profiles for top agents (by karma)
    top_agents = sorted(agents.values(), key=lambda a: a["karma"], reverse=True)[:200]
    logger.info(f"Fetching full profiles for top {len(top_agents)} agents...")
    async with httpx.AsyncClient() as client:
    ...
    if enrich_github:
        ...
        for i, (agent_id, agent) in enumerate(agents.items()):
            try:
                ...
                logger.warning(f"Enrichment error for {agent_id}: {e}")
            if i > 0 and i % 20 == 0:
                await asyncio.sleep(1)

    logger.info(f"Scrape complete: {len(agents)} agents, {len(posts_data)} posts, {len(all_projects)} projects")
